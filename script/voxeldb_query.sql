...
EXPLAIN ANALYZE SELECT * FROM voxel 
WHERE x > 100 AND x < 200 AND y > 100 AND y < 200 AND z > 100 AND z < 200;

EXPLAIN ANALYZE SELECT * FROM voxelpt 
WHERE ST_X(geom) > 100 AND ST_X(geom) < 200 AND 
ST_Y(geom) > 100 AND ST_Y(geom) < 200 AND 
ST_Z(geom) > 100 AND ST_Z(geom) < 200;

EXPLAIN ANALYZE SELECT id, classid, POINT_geom FROM (
	SELECT id, classid, (ST_Dump(geom)).geom AS POINT_geom FROM voxelmpt
) AS temp 
...
EXPLAIN ANALYZE SELECT id, classid, PCPOINT_geom FROM (
	SELECT id, classid, PC_Explode(pa) AS PCPOINT_geom FROM voxelpatch
) AS temp 
...
EXPLAIN ANALYZE SELECT * FROM voxel WHERE SQRT(POW(x-100, 2) + POW(y-200, 2) + POW(z-300, 2)) < 100;

EXPLAIN ANALYZE SELECT * FROM voxelpt WHERE SQRT(POW(ST_X(geom)-100, 2) + POW(ST_Y(geom)-200, 2) + POW(ST_Z(geom)-300, 2)) < 100;
...
EXPLAIN ANALYZE SELECT * FROM voxel WHERE classid >= 1 AND classid <= 54;
EXPLAIN ANALYZE SELECT * FROM voxelpt WHERE classid >= 1 AND classid <= 54;
EXPLAIN ANALYZE SELECT * FROM voxelmpt WHERE classid >= 1 AND classid <= 54; 
EXPLAIN ANALYZE SELECT * FROM voxelpatch WHERE classid >= 1 AND classid <= 54; 
...
WHERE classid >= 1 AND classid <= 54 AND 
SQRT(POW(PC_Get(PCPOINT_geom, 'X')-100, 2) + POW(PC_Get(PCPOINT_geom, 'Y')-200, 2) + POW(PC_Get(PCPOINT_geom, 'Z')-300, 2)) < 100;


/* Q5: Load all details for voxel (x,y,z) */
EXPLAIN ANALYZE SELECT * FROM voxel WHERE x = 100 AND y = 200 AND Z = 300;

EXPLAIN ANALYZE SELECT * FROM voxelpt WHERE ST_X(geom) = 100 AND ST_Y(geom) = 200 AND ST_Z(geom) = 300;

EXPLAIN ANALYZE SELECT id, classid, POINT_geom FROM (
	SELECT id, classid, (ST_Dump(geom)).geom AS POINT_geom FROM voxelmpt
) AS temp 
WHERE ST_X(POINT_geom)= 100 AND ST_Y(POINT_geom) = 200 AND ST_Z(geom) = 300;
...
EXPLAIN ANALYZE SELECT MIN(z) AS lowest, MAX(z) AS highest FROM (
...
EXPLAIN ANALYZE SELECT ST_ZMin(geom) AS lowest, ST_ZMax(geom) AS highest FROM (
	SELECT ST_XMin(geom) AS minX, ST_XMax(geom) AS maxX, ST_YMin(geom) AS minY, ST_YMax(geom) AS maxY 
	FROM voxelpt WHERE classid = 10
) AS temp, voxelpt 
WHERE classid = 56 AND x >= minX AND x <= maxX AND y >= minY AND y <= maxY; 

EXPLAIN ANALYZE SELECT id, classid, POINT_geom FROM (
	SELECT ST_XMin(geom) AS minX, ST_XMax(geom) AS maxX, ST_YMin(geom) AS minY, ST_YMax(geom) AS maxY 
	FROM voxelmpt WHERE classid = 10
...
EXPLAIN ANALYZE SELECT id, classid, PCPOINT_geom FROM (
	...
	FROM voxelpatch WHERE classid = 10
...
EXPLAIN ANALYZE SELECT * FROM voxel WHERE classid = 55;
EXPLAIN ANALYZE SELECT * FROM voxelpt WHERE classid = 55;
EXPLAIN ANALYZE SELECT * FROM voxelmpt WHERE classid = 55;
EXPLAIN ANALYZE SELECT * FROM voxelpatch WHERE classid = 55;


/* Q8: Load all building voxels that have door in ifc attribute */
EXPLAIN ANALYZE SELECT * FROM voxel WHERE ifcid = 8;
EXPLAIN ANALYZE SELECT * FROM voxelpt WHERE ifcid = 8;
